...
def main_single(args):
    ...
    # list dialogue acts
    comment = (labels_comment(labtor.labelset_) if args.labels_from == 'header'
               else '')
    ...
    # the naming scheme currently used for RST (2017-06-13 WIP WIP)
...
def main_pairs(args):
    ...
    if args.labels is not None:
        labelset = load_labels(args.labels, stored_as=args.labels_from)
        labels = [lbl for lbl, idx
                  in sorted(labelset.items(), key=lambda k, v: v)]
