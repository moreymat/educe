...
def config_argparser(parser):
    ...
    parser.add_argument('--labels_from',
                        choices=['header', 'file'],
                        default='header',
                        help=('Whether labels are stored as a list in the '
                              'header of an svmlight features file, or in '
                              'a standalone mapping file'))
    ...
    # end nary_enc
    # WIP 2016-07-15 same-unit (curr. disabled)
    ...
    parser.add_argument('--unordered_pairs',
                        ...
                        help=("Instances are unordered pairs: "
                              "(src, tgt) == (tgt, src)"))
    # WIP 2017-02-02 corpus- or doc-centric features file (curr. disabled)
    # extraction
...
def extract_dump_instances(docs, instance_generator, feature_set,
    ...
    if live:
        fn_out = 'extracted-features'
    else:
        fn_out = '{corpus_name}.relations'.format(
    ...
    if live:
        ...
        if labels is not None:
            labelset = load_labels(labels, stored_as=args.labels_from)
        else:
            labelset = None
    ...
    if file_split == 'doc':
        ...
        if live:
            fn_out = 'extracted-features.{fn_ext}'.format(
                fn_ext=fn_ext)
        else:
            fn_out = ('{corpus_name}.relations.{fn_ext}'
                      .format(
                          corpus_name=corpus_name,
                          fn_ext=fn_ext))
        out_file = os.path.join(out_dir, fn_out)
        dump_all(X_gen, y_gen, out_file, docs, instance_gen,
                 class_mapping=(labtor.labelset_ if args.labels_from == 'header'
                                else None))
    else:
        raise ValueError('Unknown value for args.file_split : {}'.format(
            args.file_split))
    # dump labelset
    if args.labels_from == 'file':
        if False and labels is not None:  # curr. disabled
    ...
    # dump vocabulary
    if False and vocabulary is not None:  # curr. disabled
